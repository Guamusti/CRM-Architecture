...
const companies = {
  ...
  selectColumns: `id, name, legal_name, tax_id, industry, website, email, phone,
    ...
    status, legal_basis, data_retention_until, custom, ${AUDIT_COLS}`,
  columns: ['name', 'legal_name', 'tax_id', 'industry', 'website', 'email', 'phone',
    'address', 'city', 'province', 'postal_code', 'country', 'employee_count',
    'owner_user_id', 'status', 'legal_basis', 'data_retention_until', 'custom'],
  filterable: {
...
const contacts = {
  ...
  selectColumns: `id, company_id, first_name, last_name, email, phone, job_title,
    ...
    consent_at, data_retention_until, anonymized_at, custom, ${AUDIT_COLS}`,
  columns: ['company_id', 'first_name', 'last_name', 'email', 'phone', 'job_title',
    'is_primary', 'owner_user_id', 'legal_basis', 'consent_status', 'consent_source',
    'consent_at', 'data_retention_until', 'custom'],
  filterable: {
...
const leads = {
  ...
  selectColumns: `id, title, company_id, contact_id, status, source, priority,
    owner_user_id, main_pain, product_id, estimated_value, next_step,
    next_follow_up_at, lost_reason, converted_opportunity_id, custom, ${AUDIT_COLS}`,
  columns: ['title', 'company_id', 'contact_id', 'status', 'source', 'priority',
    'owner_user_id', 'main_pain', 'product_id', 'estimated_value', 'next_step',
    'next_follow_up_at', 'lost_reason', 'custom'],
  filterable: {
    status: { column: 'status' },
...
const opportunities = {
  ...
  columns: ['title', 'company_id', 'contact_id', 'lead_id', 'stage_id', 'amount',
    ...
    'main_pain', 'product_id', 'source', 'next_step', 'next_follow_up_at', 'custom'],
  filterable: {
    status: { column: 'status' },
...
const products = {
  entityType: 'product',
  table: 'crm_products',
  selectColumns: `id, name, description, price, is_active, ${AUDIT_COLS}`,
  columns: ['name', 'description', 'price', 'is_active'],
  filterable: { is_active: { column: 'is_active' } },
  searchable: ['name', 'description'],
  sortable: { name: 'name', price: 'price', created_at: 'created_at' },
  defaultSort: 'name',
  scopeField: null,
};
...
const customFields = {
  ...
  selectColumns: `id, entity_type, key, label, field_type, options, is_required, position, ${AUDIT_COLS}`,
  columns: ['entity_type', 'key', 'label', 'field_type', 'options', 'is_required', 'position'],
  filterable: { entity_type: { column: 'entity_type' } },
  searchable: [],
  sortable: { position: 'position', label: 'label' },
  defaultSort: 'position',
  scopeField: null,
};

module.exports = { companies, contacts, leads, opportunities, stages, tasks, notes, products, customFields };
